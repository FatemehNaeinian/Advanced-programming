...
bool Flight::Is_connected(Flight& second_flight , string final_dest){

    if(destination == second_flight.get_origin() &&
        final_dest == second_flight.get_dest() &&
        (economySeats+buisnessSeats) >0  ){
        if( arrival.less_than_15hours(second_flight.get_departure_time())){
            return true;
        }
    }
    return false;
...
string Flight::get_dest(){return destination;}

string Flight::get_origin(){return origin;}

Date Flight::get_departure_time(){return departure;}

double Flight::get_economy_cost(){return economyCost;}

void Flight::calculate_show_duration(Flight* second_flight){
    arrival.show_duration(second_flight->get_departure_time());
}

void Flight::calculate_show_cost(Flight* second_flight){
    cout  << fixed << setprecision(2) <<  second_flight->get_economy_cost()+economyCost;
}

double Flight::get_connecting_cost(Flight* second_flight){
    return second_flight->get_economy_cost()+economyCost;
}

int Flight::get_num_of_sold_seats(){
    return seats-(economySeats+buisnessSeats);
}
